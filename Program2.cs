...
";
        ...
        MOB + "Assembler/BasicAssembler:AmmoMagazine,Component,GasContainerObject,Ore,OxygenContainerObject,PhysicalGunObject,Ingot/Gold,Ingot/Uranium,Ingot/Platinum\n" +
        ...
        float ExecutionTime
        {
            get { return (float)((DateTime.Now - currentCycleStartTime).TotalMilliseconds + 0.5); }
        }
        ...
        public Program()
        {
            ...
            ScreenFormatter sf = new ScreenFormatter(1, 0, Me);
            ScreenFormatter.Init();
            panelStatsHeader = (
                "Taleden's Inventory Manager\n" +
                VERSION_NICE_TEXT + "\n\n" +
                sf.Format("Run", 63, out unused, 1) +
                sf.Format("F-Step", 98 + unused, out unused, 1) +
                sf.Format("Time", 114 + unused, out unused, 1) +
                sf.Format("Load", 83 + unused, out unused, 1) +
                sf.Format("S", 51 + unused, out unused, 1) +
                sf.Format("R", 51 + unused, out unused, 1) +
                sf.Format("A", 51 + unused, out unused, 1) +
                "\n\n"
            );

            Echo(Me.GetSurface(0).SurfaceSize.X.ToString());
        ...
        public void Main(string argument, UpdateType updateSource)
        {
            ...
            float exTime = (float)Math.Truncate(ExecutionTime * 100) / 100;
            double exLoad = Math.Round(100.0f * ExecutionLoad, 1);
            int unused;
            ScreenFormatter sf = new ScreenFormatter(1, 1, Me);
            statsLog[totalCallCount % statsLog.Length] = (
                sf.Format("" + totalCallCount, 63, out unused, 1) +
                sf.Format((processStep == 0 ? processSteps.Length : processStep) + " / " + processSteps.Length, 98 + unused, out unused, 1, true) +
                sf.Format(exTime + " ms", 114 + unused, out unused, 1) +
                sf.Format(exLoad + "%", 83 + unused, out unused, 1, true) +
                sf.Format("" + numberTransfers, 51 + unused, out unused, 1, true) +
                sf.Format("" + numberRefineres, 51 + unused, out unused, 1, true) +
                sf.Format("" + numberAssemblers, 51 + unused, out unused, 1, true) +
                "\n"
            );
        ...
        void ParseBlockTags()
        {
            ...
            foreach (IMyTerminalBlock block in blockTag.Keys)
            {
                ...
                {
                    sb.Append(block.CustomName, 0, match.Index);
                    sb.Append(argTagOpen);
                    if (argTagPrefix != "")
                        sb.Append(argTagPrefix + " ");
                }
                    ...
                    foreach (string a in attrs)
                    {
                        ...
                        else if (attr == "SPAN")
                        {
                            if (fields.Length >= 3 && (int.TryParse(fields[1], out spanwide) & int.TryParse(fields[2], out spantall) & spanwide >= 1 & spantall >= 1))
                            {
                                ...
                                sb.Append("SPAN:" + spanwide + ":" + spantall + " ");
                            }
                            else
                            {
                                sb.Append((attr = String.Join(":", fields).ToLower()) + " ");
                        ...
                        else if (attr.Length >= 3 & "QUOTAS".StartsWith(attr))
                        {
                            ...
                            while (++i < fields.Length)
                            {
                                ...
                                else if (fields[i].StartsWith("P") & int.TryParse(fields[i].Substring(Math.Min(1, fields[i].Length)), out priority))
                                {
                                    ...
                                    if (priority > 0) sb.Append(":P" + priority);
                                }
                                else
                                {
                                    sb.Append(":" + fields[i].ToLower());
                ...
                else
                {
                    ...
                    foreach (string a in attrs)
                    {
                            ...
                            while (++i < fields.Length)
                            {
                                if (ParseItemTypeSub(null, true, fields[i], (blkRfn != null) ? "ORE" : "", out itype, out isub) & (blkRfn != null) == (itype == "ORE") & (blkRfn != null | itype != "INGOT"))
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        sb.Append(":" + ((blkRfn == null & subTypes[isub].Count > 1) ? (typeLabel[itype] + "/") : "") + subLabel[isub]);
                                    }
                                }
                                else
                                {
                        ...
                        {
                            sb.Append(String.Join(":", fields).ToLower() + " ");
                        ...
                        else
                        {
                            ...
                            if (argRewriteTags & !grouped)
                            {
                                if (force)
                                {
                                    sb.Append("FORCE:" + typeLabel[itype]);
                                    if (isub != "")
                                        sb.Append("/" + subLabel[isub]);
                                }
                                else if (isub == "")
                                {
                                    sb.Append(typeLabel[itype]);
                                ...
                                {
                                    sb.Append(typeLabel[itype] + "/" + subLabel[isub]);
                                }
                                if (priority > 0 & priority < int.MaxValue)
                                    sb.Append(":P" + priority);
                                if (amount >= 0L)
                                    sb.Append(":" + (amount / 1e6));
                                sb.Append(" ");
                            }
                        }
                    }
                ...
                if (argRewriteTags & !grouped)
                {
                    if (sb[sb.Length - 1] == ' ')
                        sb.Length--;
                    sb.Append(argTagClose).Append(block.CustomName, match.Index + match.Length, block.CustomName.Length - match.Index - match.Length);
                    block.CustomName = sb.ToString();
                }
        ...
        void UpdateInventoryPanels()
        {
            ...
            foreach (List<IMyTextPanel> panels in itypesPanels.Values)
            {
                ...
                sf.SetWidth(3, sf.GetWidth(new StringBuilder("8.88" + ((maxamt >= 1000000000000L) ? " M" : ((maxamt >= 1000000000L) ? " K" : ""))), true));
                sf.SetWidth(5, sf.GetWidth(new StringBuilder("8.88" + ((maxqta >= 1000000000000L) ? " M" : ((maxqta >= 1000000000L) ? " K" : ""))), true));
        ...
        void WriteTableToPanel(string title, ScreenFormatter sf, IMyTextPanel panel, bool allowspan = true, string before = "", string after = "")
        {
            ...
            width = 512 * wide; // TODO monospace 26x17.5 chars
        ...
        public class ScreenFormatter
        {
            ...
            private static Dictionary<StringBuilder, int> textWidth = new Dictionary<StringBuilder, int>();
            private static byte SZ_SPACE;
            private static byte SZ_SHYPH;

            public /*static*/ int GetWidth(StringBuilder text, bool memoize = false, float size = 1)
            {
                ...
                if (!textWidth.TryGetValue(text, out width))
                {
                    ...
                    while (i > 0)
                    {
                        ...
                        width += (int)me.GetSurface(0).MeasureStringInPixels(text, "Debug", size).X;
                        i -= 8;
                    }
                    if (memoize)
                ...
                //  return (int)me.GetSurface(0).MeasureStringInPixels(text, "Debug", size).X;
            } // GetWidth()
            ...
            public /*static*/ string Format(string text, int width, out int unused, int align = -1, bool memoize = false)
            {
                ...
                unused = width - GetWidth(sb, memoize);
                ...
                if (align > 0)
                    return new String(' ', spaces) + new String('\u00AD', bars) + sb;
                if (align < 0)
                    return sb + new String('\u00AD', bars) + new String(' ', spaces);
                if ((spaces % 2) > 0 & (bars % 2) == 0)
                    return new String(' ', spaces / 2) + new String('\u00AD', bars / 2) + sb + new String('\u00AD', bars / 2) + new String(' ', spaces - (spaces / 2));
                return new String(' ', spaces - (spaces / 2)) + new String('\u00AD', bars / 2) + sb + new String('\u00AD', bars - (bars / 2)) + new String(' ', spaces / 2);
            } // Format()
            ...
            public ScreenFormatter(int numCols, int padding = 1, IMyProgrammableBlock Me = null)
            {
                this.me = Me;
            ...
            {
                int width = 0;
                this.colRowText[col].Add(text.ToString());
                if (this.colBar[col] == false)
                {
                    width = GetWidth(new StringBuilder(text), memoize);
